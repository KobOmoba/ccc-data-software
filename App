 ...
 * Equipped with self-healing schema sanitizers for mobile testing.
 */
...
window.onload = async function() {
    ...
    if (savedState) {
        try {
            ...
            if (parsed && typeof parsed === "object") {
                // Self-Heal: Merge older schemas into modern structures to prevent load crashes
                state = { ...state, ...parsed };
            }
        } catch(e) {
    ...
    // Force compile safety checks against legacy missing database arrays
    if (!Array.isArray(state.parishes)) state.parishes = [];
    if (!Array.isArray(state.members)) state.members = [];
    if (!Array.isArray(state.linkages)) state.linkages = [];
    if (!Array.isArray(state.attendance)) state.attendance = [];
    if (!Array.isArray(state.finances)) state.finances = [];
    if (!Array.isArray(state.births)) state.births = [];
    if (!Array.isArray(state.deaths)) state.deaths = [];
    if (!Array.isArray(state.committees)) state.committees = [];
    
    if (!state.departments || typeof state.departments !== "object") {
        state.departments = {
            "THE PAROCHIAL COMMITTEE": { current: [], former: [] },
            "THE CHOIR": { current: [], former: [] },
            "THE ICT MEDIA": { current: [], former: [] },
    ...
    if (state.parishes.length === 0 || !state.parishes[0].password) {
        state.parishes = [{
...
// ==========================================
// 13. COMMUNICATION PORT & DELETIONS
// ==========================================
...
function applyConfigToDOM() {
    ...
    if (state.logoBase64) {
        ...
        resetLogo("landingLogoImg", "landingLogoPlaceholder");
        resetLogo("dashboardLogoImg", "dashboardLogoPlaceholder");
        resetLogo("mobileLogoImg", "mobileLogoPlaceholder");
    }
}

function calculateLiturgicalDates() {
    const currentYear = new Date().getFullYear();
    let juvenileDate = new Date(currentYear, 5, 1); 
    while (juvenileDate.getDay() !== 0) {
        juvenileDate.setDate(juvenileDate.getDate() + 1);
    }

    document.getElementById("calJuvenileDate").innerText = "Sunday, " + juvenileDate.toDateString();
    
    if (state.adultHarvestDate) {
        const d = new Date(state.adultHarvestDate);
        document.getElementById("calAdultDate").innerText = d.toDateString();
    } else {
        document.getElementById("calAdultDate").innerText = "Not Selected (Edit in Settings)";
    }
}
...
async function attemptLogin() {
    ...
    const entranceType = document.getElementById("entranceSelector").value;
    
    if (entranceType === 'aarinat') {
        if (passwordEntered.toLowerCase() === 'aarinat') { 
            state.isLoggedIn = true;
            state.activeParishId = 'aarinat';
            saveStateToStorage();
            ...
            alert("Platform Super-Admin validation failed.");
        }
        return;
    }

    const activeParish = state.parishes.find(p => p.id === entranceType);
    if (activeParish) {
        if (activeParish.status === 'Pending') {
            alert(`Access Denied:\n"${activeParish.name}" license pending activation.`);
            return;
        }

        if (passwordEntered.toLowerCase() === activeParish.password.toLowerCase() || passwordEntered.toLowerCase() === 'admin') {
            state.isLoggedIn = true;
            state.currentParishId = activeParish.id;
            state.parishName = activeParish.name;
            state.adminPassword = activeParish.password;
            state.activeParishId = activeParish.id;
            saveStateToStorage();
            ...
            alert("Validation credentials incorrect.");
        }
    }
}
...
function processAgeTransitions() {
    ...
    state.members.forEach(m => {
        if (m.classification === "MEMBER" && m.dobYear) {
            ...
            if (age !== null && age >= 18 && m.isYouthPipeline) {
                delete m.isYouthPipeline;
                stateChanged = true;
                console.log(`Auto-Promoted ${m.name} to Adult Registry.`);
            }
        }
    });
...
function renderWeeklyCelebrations() {
    const container = document.getElementById("weeklyCelebrationsList");
    if (!container) return;
    container.innerHTML = "";

    const activeCelebrants = state.members.filter(m => m.dob);
    if (activeCelebrants.length === 0) {
        container.innerHTML = `<p class="text-xs text-gray-400">No birthdays recorded.</p>`;
        return;
    }

    activeCelebrants.forEach(m => {
        const ageText = m.dobYear ? `(Age: ${getCalculatedAge(m.dobYear)})` : "";
        container.innerHTML += `
            <div class="p-3 bg-gray-50 border rounded-lg flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 text-xs">
                <div>
                    <p class="font-bold text-gray-800">${m.name} <span class="text-blue-600 font-normal">${ageText}</span></p>
                    <p class="text-[10px] text-gray-500">Anniversary: <strong class="text-pink-600">${m.dob}</strong></p>
                </div>
                <div class="flex gap-1">
                    <button onclick="triggerAICelebMsg('${m.id}', 'WhatsApp')" class="px-2.5 py-1.5 bg-emerald-100 text-emerald-700 rounded text-[10px] font-bold hover:bg-emerald-200">
                        <i class="fa-brands fa-whatsapp"></i> WhatsApp
                    </button>
                    <button onclick="triggerAICelebMsg('${m.id}', 'Email')" class="px-2.5 py-1.5 bg-blue-100 text-blue-700 rounded text-[10px] font-bold hover:bg-blue-200">
...
function triggerAICelebMsg(memberId, channel) {
    const member = state.members.find(m => m.id === memberId);
    if (!member) return;

    const birthdayMsg = `Celestial Church of Christ: Happy Birthday anniversary to our beloved ${member.rank || 'member'}, ${member.name}. May the light of Jehovah continue to shine upon your path. Halleluyah!`;
    
    if (channel === 'WhatsApp') {
        const encodedMsg = encodeURIComponent(birthdayMsg);
        const wpUrl = member.phone ? `https://wa.me/${member.phone.replace(/[^0-9]/g, '')}?text=${encodedMsg}` : `https://wa.me/?text=${encodedMsg}`;
        window.open(wpUrl, '_blank');
    } else {
        const mailUrl = `mailto:${member.phone || 'info@cccparish.org'}?subject=Happy Birthday&body=${encodeURIComponent(birthdayMsg)}`;
        window.open(mailUrl, '_blank');
    }
}

function renderHQApprovals() {
    const tbody = document.getElementById("approvalsTableBody");
    if (!tbody) return;
    tbody.innerHTML = "";

    if (state.parishes.length <= 1) {
        tbody.innerHTML = `<tr><td colspan="4" class="p-4 text-center text-gray-400">No other parish validation entries found.</td></tr>`;
        return;
    }

    state.parishes.forEach(p => {
        if (p.id === state.parishes[0].id) return; // Ignore seed parish

        tbody.innerHTML += `
            <tr class="text-xs">
                <td class="p-3 font-bold text-slate-800">${p.name}</td>
                <td class="p-3 font-mono text-gray-500">${p.id}</td>
                <td class="p-3 font-semibold ${p.status === 'Approved' ? 'text-emerald-600' : 'text-amber-500 animate-pulse'}">${p.status}</td>
                <td class="p-3 text-right">
                    ${p.status === 'Pending' ? 
                      `<button onclick="approveParishLicense('${p.id}')" class="px-2.5 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded font-bold">Approve</button>` : 
                      `<button onclick="suspendParishLicense('${p.id}')" class="px-2.5 py-1.5 bg-red-100 hover:bg-red-200 text-red-700 rounded font-bold">Suspend</button>`
                    }
                </td>
            </tr>
        `;
    ...
    if (parish) {
        ...
        if (supabaseClient) {
            try {
                ...
                console.error("Cloud status update deferred:", e);
            }
        }
        
        renderHQApprovals();
        alert(`Licensing active for "${parish.name}".`);
    }
}
...
async function suspendParishLicense(id) {
    ...
    if (parish) {
        ...
        if (supabaseClient) {
            try {
                await supabaseClient.from('parishes').upsert([{ id: id, status: 'Pending' }]);
            } catch (e) {
                console.error("Cloud status update deferred:", e);
            }
        }
        ...
        renderHQApprovals();
        alert(`Licensing suspended for "${parish.name}".`);
    }
}
