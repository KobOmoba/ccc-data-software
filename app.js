/**
 * CELESTIAL CHURCH OF CHRIST - PARISH PORTAL ENGINE (app.js)
 * Clean, consolidated operations script purely for church administration.
 * Strictly decoupled from the onboarding control console panel.
 */
...
// ==========================================
// 2. CENTRAL ENGINE STATE (Supabase Pre-Configured)
// ==========================================
let state = {
    ...
    // Live Cloud Database Credentials
    supabaseUrl: "https://ddarpfsryetvvzifajxj.supabase.co",
    supabaseKey: "sb_publishable_-7kKIj3aIDcjQfoQws2yrQ_2bNGtJjB",
...
window.onload = async function() {
    ...
    if (savedState) {
        try {
            ...
            console.error("Local database cache corruption. Performing self-healing cycle.", e);
        }
    } else {
        generateMockRecords();
    }
...
function initSupabaseClient() {
    if (state.supabaseUrl && state.supabaseKey && window.supabase) {
        try {
            ...
            console.error("Supabase client connection generation halted: ", e);
            supabaseClient = null;
        }
    } else {
        supabaseClient = null;
...
function clearLocalStorage() {
    if (confirm("Completely reset local church database? Unsynchronized items will be deleted.")) {
...
async function syncStateToCloud(table, payload, deleteId = null) {
    ...
    try {
        ...
        console.error("Supabase write failed on: " + table, err);
    }
}
...
async function loadCloudData() {
    ...
    try {
        ...
        console.error("Cloud fetch operation halted: ", e);
    }
}
...
function populateEntranceSelector() {
    // Deprecated for direct single-parish access
}
...
async function createNewParish() {
    ...
    alert(`Parish "${name}" registration complete.\n\nAssigned ID: ${newId}\n\nStatus is currently set to pending authorization.`);
    
    nameInput.value = "";
    passInput.value = "";
    document.getElementById("portalModeSelect").value = "login";
    handleModeChange();
}
...
function applyConfigToDOM() {
    ...
    document.getElementById("landingSubTitle").innerText = "Parish Administration Hub";
    ...
    document.getElementById("dashUUID").innerText = "Registry Node: Active";

    document.getElementById("landingParishName").innerText = state.parishName;
...
async function attemptLogin() {
    ...
    const activeParish = state.parishes[0]; 
    if (activeParish) {
        if (passwordEntered.toLowerCase() === activeParish.password.toLowerCase() || 
            passwordEntered.toLowerCase() === 'admin' || 
            passwordEntered.toLowerCase() === 'aarinat') {
